        ...
        protected ArrayList<Bookings> doInBackground(Void... voids) {
            ...
            try {
                ...
                String queryCities = String.format(
                        "select `reservations`.`startDateTime`, `reservations`.`endDateTime`, `reservations`.`invoiceId`, `parkingLot`.`cityName`, `parkingLot`.`buildingName`, `parkingLot`.`parkingId`, `reservations`.`rowId`, `reservations`.`columnId`\n" +
                                "from `reservations`\n" +
